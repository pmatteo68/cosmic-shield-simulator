...
//std::map<G4String, G4int> CsvHelper::tupleNameToId;
//std::mutex CsvManager::mutex;
//G4int CsvHelper::tupleCounter = 0;

//G4AnalysisManager* CsvHelper::getManager() {
...
G4AnalysisManager* CsvHelper::getManager() {
    thread_local G4AnalysisManager* cachedAm = [] {
        ...
        //G4cout << "[CsvHelper] Initialized analysis manager for thread ID "
...
void CsvHelper::openFile(const G4String& fileName) {
    auto* am = getManager();
...
G4int CsvHelper::createTuple(const G4String& tupleIdString, const G4String& tupleDesc, const std::vector<std::pair<G4String, char>>& columns) {
...
//void CsvHelper::addDataField(int fldPos, G4String s) {
//    auto* am = getManager();
//    am->FillNtupleSColumn(fldPos, s);
//}

//void CsvHelper::addDataField(int fldPos, G4double d) {
//    auto* am = getManager();
//    am->FillNtupleDColumn(fldPos, d);
//}

void CsvHelper::addDataField(int fldPos, std::string s) {
    auto* am = getManager();
    am->FillNtupleSColumn(fldPos, s);
}

void CsvHelper::addDataField(int fldPos, char c) {
    auto* am = getManager();
    am->FillNtupleSColumn(fldPos, std::string(1, c));
}

void CsvHelper::addDataField(int fldPos, int i) {
    auto* am = getManager();
    am->FillNtupleDColumn(fldPos, static_cast<double>(i));
}

//void CsvHelper::addDataField(int fldPos, G4int i) {
//    auto* am = getManager();
//    am->FillNtupleDColumn(fldPos, static_cast<double>(i));
//}

void CsvHelper::addDataField(int fldPos, double d) {
    auto* am = getManager();
    am->FillNtupleDColumn(fldPos, d);
}

void CsvHelper::addRecord() {
    auto* am = getManager();
    am->AddNtupleRow();
}

/*
void CsvHelper::addRXecord(const CsvFieldVector& fields) {
    auto* am = getManager();
    unsigned fIdx = 0;
...
//G4int CsvHelper::getID(const G4String& tupleIdString) {
//    std::lock_guard<std::mutex> lock(mutex);
